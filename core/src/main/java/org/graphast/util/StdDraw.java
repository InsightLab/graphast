...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	 ...
	 * 
	 * // from a file or URL try { URL url = new URL(filename); BufferedImage
	 * image = ImageIO.read(url); return image; } catch (IOException e) { //
	 * ignore }
	 * 
	 * // in case file is inside a .jar (classpath relative to StdDraw) try {
	 ...
	 * { URL url = StdDraw.class.getResource("/" + filename); BufferedImage
	 * image = ImageIO.read(url); return image; } catch (IOException e) { //
	 * ignore } throw new IllegalArgumentException("image " + filename +
	 * " not found"); }
	 */
	/**
	 ...
	 * @throws IllegalArgumentException
	 *             if either {@code scaledWidth} or {@code scaledHeight} is
	 *             negative
	 * @throws IllegalArgumentException
	 *             if the image filename is invalid
	 */
	 ...
	 * {@code line()}, {@code circle()}, and {@code square()} will be deffered
	 ...
	 * {@code line()}, {@code circle()}, and {@code square()} will be displayed
	...
	public static void drawGraph(CHGraph graph) {

		StdDraw.setCanvasSize(1280, 720);
		StdDraw.setXscale(graph.getBBox().getMinLatitude()*1.1, graph.getBBox().getMaxLatitude()*1.1);
		StdDraw.setYscale(graph.getBBox().getMinLongitude()*1.1, graph.getBBox().getMaxLongitude()*1.1);
//		StdDraw.setScale(-40, 170);
		StdDraw.setPenColor(StdDraw.BLACK);
...
//				StdDraw.setPenColor(StdDraw.RED);
			...
			Node fromNode = graph.getNode(graph.getEdge(i).getFromNode());
			Node toNode = graph.getNode(graph.getEdge(i).getToNode());
		...
		for (int i = 0; i < graph.getNumberOfNodes(); i++) {
			StdDraw.point(graph.getNode(i).getLatitude(), graph.getNode(i).getLongitude());
			StdDraw.setPenColor(StdDraw.WHITE);
			StdDraw.text(graph.getNode(i).getLatitude(), graph.getNode(i).getLongitude(), graph.getNode(i).getId().toString());
			StdDraw.setPenColor(StdDraw.BLACK);
		}
